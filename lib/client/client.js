...
function Client(options) {
  ...
  this.saslFactory = options.saslFactory || null;
 ...
 * @param {Control} controls (optional) either a Control or [Control].
 * @param {String} mechanish (optional) name of the Authentication Mechanism.
 * @param {Function} callback of the form f(err, res).
 * @throws {TypeError} on invalid input.
 */
Client.prototype.bind = function bind(name, credentials, controls, mechanism, callback) {
  ...
  if (typeof (controls) === 'function') {
    ...
    mechanism = controls;
    controls = [];
  } else if(typeof (mechanism) === 'function') {
    callback = mechanism;
  } else {
    controls = validateControls(controls);
  }
  ...
  if (typeof (mechanism) === 'string') {
      // Validate Auth Mechanism is supported...

      // TODO ...
  } else {
      mechanism = 'Simple'; // DEFAULT TO 'Simple' AUTH...
    // Invalid Auth Mechanism ERR!
    //throw new TypeError('(optional) mechanism must be of type (string)');
  }
...
Client.prototype._connect = function _connect() {
  ...
  if (this.connectTimeout) {
    timer = setTimeout(function onConnectTimeout() {
      if( socket ) {
        // FORCE 'connectTimeout' SOCKET ERROR...
        self.socket.emit('error','connectTimeout');
      }
    }, this.connectTimeout);
  }
  socket.once('error', function() { clearTimeout(timer); });
