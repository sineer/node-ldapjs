    ...
    create: function createClient(callback) {
      ...
      client.once('connect', function onConnect() {
        ...
        return client.bind(options.bindDN,
                           ...
                           options.bindAuthentication || 'Simple',
                           bindCallback);
      });
    },
...
function ClientPool(options) {
  ...
  this.options = {
    ...
    bindAuthentication: options.bindAuthentication || '',
    saslFactory: options.saslFactory || null,
    saslMechanism: options.saslFactory || '',
    checkInterval: options.checkInterval,
...
ClientPool.prototype.bind = function bind(dn, creds, controls, mechanism, callback) {
  if (typeof (controls) === 'function') {
    callback = controls;
    mechanism = 'Simple';
    controls = [];
  ...
  self.options.bindAuthentication = null;
  ...
  return this.pool.shutdown(function () {
    ...
    return self.pool.acquire(function onAcquire(err, client) {
      ...
      return client.bind(dn, creds, controls, mechanism, function (err, res) {
        self.pool.release(client);
        ...
        self.options.bindAuthentication = mechanism;
