...
function BindRequest(options) {
  ...
  this.authentication = options.authentication || LDAP_BIND_SIMPLE; // SASL AUTH MECHANISM
  this.credentials    = options.credentials || '';

  if (this.authentication != LDAP_BIND_SIMPLE) {

    // Try using JS-SASL FACTORY to instanciate mechanism...
    this.mechanism = SASLFactory.create(this.authentication);
    if ( !this.mechanism ) {
...
BindRequest.prototype._toBer = function (ber) {
  ...
  case LDAP_BIND_GSSAPI:
    ...
    ber.writeString(this.credentials, Ber.Context); // SASL CREDENTIALS
    ber.endSequence();
    break;
